...
| `pause-before-nuclear-fanout` | Prices any multi-agent launch and asks before spending (a PreToolUse hook refuses a Workflow with no typed estimate); runs the same crux-and-advocates protocol inline with zero agents for any decide-between or is-this-true request; re-checks at every phase boundary and kills the run once the answer is visible; when a deliberation does run it is cruxes first (fact vs preference, shown to the user), priors before evidence, base rates before specifics, two advocates in a short serial exchange, double-crux on what remains, the orchestrator as judge, stop on information not rounds; the deliverable is a sub-400-word plain answer, and facts about the user travel with their source or not at all | A two-option decision the user had already made: 59 agents, 12.7M tokens, a 6,000-word report they had to ask to have explained, and a panel inference reported back to them as their own words |
...
| `rehearsing-against-a-counterparty` | Runs the real artifact against N independent simulated counterparties built from documented behaviour, with your own replies scripted and the opponent hardened each round; its central finding is that the reply protocol wins the headline number while the document wins every term underneath it | 31 runs across 8 rounds rehearsing a one-shot counter-offer, which reversed five of the assistant's own recommendations before the message was sent |
| `training-the-reader` | A self-improving cold-read loop for teaching artifacts: a defect catalog every reader reads first and appends to, a plain-prose gate script (sentence cap, term-before-definition, number-without-origin, frozen code/figures/tables), and a promotion rule that turns any class seen twice into a check; cheap models do the reading and rewriting, the frontier writes only the analogy seeds | A 21-file interview-prep library the maintainer called "verbal insanity": four cold reads re-derived the same defect classes from scratch, a rewriter died emitting one 34 KB file, and nothing carried forward between reads |
