...
| `pause-before-nuclear-fanout` | Prices any multi-agent launch and asks before spending; re-checks at every phase boundary and kills the run once the answer is visible; when a panel does run it is three to five lenses on per-lens slices told to attack the current lean, 250-word memos, no round 2 and no moderator agent; the deliverable is a sub-400-word plain answer, and facts about the user travel with their source or not at all | A two-option decision the user had already made: 59 agents, 12.7M tokens, a 6,000-word report they had to ask to have explained, and a panel inference reported back to them as their own words |
